  ...
  if (!mockAlumno) {
    return (
      <div className="p-8 text-center bg-slate-900 border border-slate-800 rounded-3xl">
        ...
        <p className="text-slate-400 mt-2">No se pudieron cargar los datos del expediente académico.</p>
      </div>
    );
  }
    ...
    <div className="space-y-8 animate-fadeIn">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex items-center gap-4">
          <Link href="/dashboard/perfil" className="p-2 bg-slate-900 border border-slate-800 rounded-xl text-slate-400 hover:text-white transition-colors">
            <ArrowLeft size={20} />
          </Link>
          <div>
            <h1 className="text-3xl font-bold text-white tracking-tight">Kardex Académico</h1>
            <p className="text-slate-400 text-sm mt-1">Historial oficial de calificaciones y rendimiento.</p>
          </div>
        </div>

        <button className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-lg active:scale-95">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-6 relative overflow-hidden group">
          ...
          <p className="text-xs text-slate-500 uppercase font-bold tracking-widest mb-1">Promedio General</p>
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-6 relative overflow-hidden group">
          ...
          <p className="text-xs text-slate-500 uppercase font-bold tracking-widest mb-1">Materias Aprobadas</p>
          <p className="text-4xl font-black text-white">{materiasAprobadas}<span className="text-xl text-slate-600 font-bold ml-1">/ {mockAlumno.calificaciones.length}</span></p>
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-6 lg:col-span-2">
          <div className="flex gap-4 items-center">
            <div className="w-12 h-12 bg-slate-950 border border-slate-800 rounded-2xl flex items-center justify-center text-orange-400">
              <User size={24} />
            </div>
            <div>
              <p className="text-lg font-bold text-white">{mockAlumno.apellidoPaterno} {mockAlumno.apellidoMaterno} {mockAlumno.nombres}</p>
              <p className="text-xs text-slate-400 font-mono">{mockAlumno.matricula} · {mockAlumno.grupoRel?.nombre || mockAlumno.grupo} · {mockAlumno.turno}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-[2rem] overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-950/30 text-slate-400 text-xs uppercase tracking-widest border-b border-slate-800">
              <tr>
            ...
            <tbody className="divide-y divide-slate-800/50 text-slate-300">
              {mockAlumno.calificaciones.length === 0 ? (
                ...
                mockAlumno.calificaciones.map((cal) => (
                  <tr key={cal.id} className="hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center text-slate-400 font-medium">{cal.parcial1 ?? '—'}</td>
                    <td className="px-6 py-4 text-center text-slate-400 font-medium">{cal.parcial2 ?? '—'}</td>
                    <td className="px-6 py-4 text-center text-slate-400 font-medium">{cal.parcial3 ?? '—'}</td>
                    <td className="px-6 py-4 text-center">
                      <span className={`text-lg font-black ${(cal.final || 0) >= 6 ? 'text-emerald-400' : 'text-red-400'}`}>
                        {cal.final?.toFixed(1) || '—'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-center">
